...
export const ImportUploadResponse = t.Object({
  tempFileId: t.String({ description: 'Temporary file ID for import operations' }),
  filename: t.String({ description: 'Original filename' }),
  size: t.Number({ description: 'File size in bytes' }),
});

// Import metadata response
export const ImportMetadataResponse = t.Object({
  metadata: t.Object({
    serverType: t.Optional(t.String({ description: 'Detected server type' })),
    minecraftVersion: t.Optional(t.String({ description: 'Detected Minecraft version' })),
    hasWorld: t.Boolean({ description: 'Whether backup contains world data' }),
    hasPlugins: t.Boolean({ description: 'Whether backup contains plugins' }),
    hasMods: t.Boolean({ description: 'Whether backup contains mods' }),
  }),
});

// Import diff request
export const ImportDiffBody = t.Object({
  tempFileId: t.String({ description: 'Temporary file ID' }),
});

// Import apply request
export const ImportApplyBody = t.Object({
  tempFileId: t.String({ description: 'Temporary file ID' }),
  selections: t.Array(t.Object({
    id: t.String({ description: 'Diff item ID' }),
    action: t.Union([t.Literal('keep'), t.Literal('use-backup')], { description: 'Action to take' }),
  })),
});

// Import apply response
export const ImportApplyResponse = t.Object({
  success: t.Boolean({ description: 'Operation success' }),
  applied: t.Number({ description: 'Number of changes applied' }),
  skipped: t.Number({ description: 'Number of changes skipped' }),
});
