...
class ProcessManager extends EventEmitter {
  ...
  constructor() {
    super()
    this.on('error', (_serverId: string, _err: Error) => {})
  }
  ...
  async start(server: Server): Promise<void> {
    ...
    proc.stdout?.on('data', (data: Buffer) => {
      ...
      for (const line of lines) {
        ...
        if (line.includes('Done') && line.includes('For help, type')) {
          info.state = 'running'
          log.info({ serverId: server.id, pid: proc.pid }, 'Server is running')
    ...
    proc.on('exit', (code, signal) => {
      ...
      if (code === 0 || info.state === 'stopping') {
        info.state = 'stopped'
        log.info({ serverId: server.id, exitCode: code }, 'Server process stopped')
      } else {
        info.state = 'crashed'
        info.lastError = `Process exited with code ${code}, signal ${signal}`
        log.error({ serverId: server.id, exitCode: code, signal }, 'Server process crashed')
      }
    ...
    proc.on('error', (err) => {
      ...
      log.error({ serverId: server.id, err }, 'Server process error')
